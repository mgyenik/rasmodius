 ...
 */
export function flattenConditions(node: FilterCondition | FilterGroup): FilterCondition[] {
...
function extractGeodeHighlights(
	...
	const geodeConditions = conditions.filter(
		(c): c is FilterCondition & { type: 'geode' } => c.type === 'geode' && c.geodeType === geodeType
	);
